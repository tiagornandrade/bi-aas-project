...
from commons.accounts import AccountEventsService
...
class AccountService:
    ...
    def insert_users(count: int):
        ...
        db = SessionLocal()
        try:
            user_dicts = AccountEventsService.generate_users(count)
    ...
    def insert_accounts(count: int):
        ...
        try:
            ...
            account_dicts = AccountEventsService.generate_accounts(count)
            valid_accounts = [
            ...
            return db.query(Account).order_by(Account.account_id.desc()).limit(count).all()
        except Exception as e:
            db.rollback()
    ...
    def insert_subaccounts(count: int):
        ...
        try:
            ...
            subaccount_dicts = AccountEventsService.generate_subaccounts(count)
            valid_subaccounts = [
                Subaccount(**{**subaccount, "parent_account_id": np.random.choice(account_ids)})
            ...
            return db.query(Subaccount).order_by(Subaccount.subaccount_id.desc()).limit(count).all()
        except Exception as e:
            db.rollback()
