...
def get_children_sum(children, attr, root):
    return reduce(
        (lambda sum, childId: sum + root[childId]["meta"][attr]), children, 0
    )  # noqa: N803
...
def get_wide_column_ids(children, root):
    return list(
        filter(
            lambda childId: can_reduce_column_width(root[childId], root), children
        )  # noqa: N803
    )
        ...
        if ROW_TYPE == item["type"]:
            ...
            if meta.get("width", 0) > GRID_COLUMN_COUNT:
                ...
                while current_width > GRID_COLUMN_COUNT and len(
                    list(
                        filter(
                            lambda childId: is_wide_leaf_component(
                                root[childId]
                            ),  # noqa: N803
                            item["children"],
                        )
                ...
                if current_width > GRID_COLUMN_COUNT:
                    ...
                    while has_wide_columns:
                        ...
                        while idx < len(col_ids) and current_width > GRID_COLUMN_COUNT:
                            ...
                            for childId in root[current_column][
                                "children"
                            ]:  # noqa: N806
